...
| 분류         | 기술                | 선택 이유                                                                |
| ------------ | ------------------- | ------------------------------------------------------------------------ |
| 언어         | Kotlin 2.2 (JVM 21) | Null 안정성과 간결한 문법으로 보일러플레이트를 줄이고 런타임 안정성 확보 |
...
> 테스트 코드는 [Claude Code](https://claude.com/claude-code)를 활용해 작성했습니다.

JUnit5 + MockK + AssertJ + Testcontainers(실제 PostgreSQL / Redis)를 기반으로 레이어별 테스트를 갖췄습니다.
H2, Mockito는 Kotlin final 클래스와 Postgres 전용 SQL에 부적합하여 사용하지 않습니다.

| 레이어          | 도구                               | 검증 대상                                                                      |
| --------------- | ---------------------------------- | ------------------------------------------------------------------------------ |
| 단위            | MockK                              | application 서비스 로직(생성, 삭제, 페이지네이션, 포인트 차감, 이벤트 발행 등) |
| 웹 슬라이스     | `@WebMvcTest` + MockMvc            | 컨트롤러 라우팅, `@Login` 인증 · 요청 검증, ErrorCode > HttpStatus 매핑        |
| 영속성 슬라이스 | `@DataJpaTest` + Testcontainers    | JDSL 키셋 커서 쿼리의 정렬 · 경계, 소프트 삭제 필터                            |
| 통합            | `@SpringBootTest` + Testcontainers | Flyway 마이그레이션 적용 + 전체 컨텍스트 부팅                                  |

- 실패 단언은 메시지가 아닌 `ErrorCode`로 검증 - iOS 클라이언트가 의존하는 안정적인 계약을 보장합니다.
- Testcontainers로 실제 PostgreSQL / Redis 위에서 마이그레이션, 키셋 쿼리, 소프트 삭제까지 검증합니다.
